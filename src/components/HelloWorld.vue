...
<template>
  ...
  <h2>{{ state.count }}</h2>
  <h3>{{ state.double }}</h3>
  <van-button type="primary" @click="add">+count</van-button>
  <van-cell-group>
    <van-cell title="单元格" value="内容"></van-cell>
    <van-cell title="单元格" value="内容" label="描述信息"></van-cell>
  </van-cell-group>
  <div>
    <input type="number" v-model="stateCount.num1" />
    <span>+</span>
    <input type="number" v-model="stateCount.num2" />
    <span>=</span>
    {{ stateCount.result }}
  </div>
  <div><van-button>Hello</van-button></div>
</template>
